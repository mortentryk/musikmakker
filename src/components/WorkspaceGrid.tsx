...
export function WorkspaceGrid({
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col">
      <div className="ejay-inset relative m-2 flex min-h-0 flex-1 flex-col overflow-hidden">
        <div
          className="pointer-events-none absolute bottom-0 top-0 z-10 ejay-playhead-col transition-all duration-75"
          style={{
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center justify-center gap-1 border-r border-[#1e3a5f] bg-[#0a1020]/70 py-1">
                  <button
                    type="button"
                    className={`ejay-btn h-7 w-8 px-0 text-[11px] ${theme.categoryAccent}`}
...
function GridCell({
  ...
  colIndex: number;
  blockId: string | null;
  theme: (typeof EJAY_TRACK)[keyof typeof EJAY_TRACK];
  playheadColumn: number;
  aiHint: AiDropHint;
  feedback: GameFeedback;
  getSlotLabel: (blockId: string | null) => string | null;
            ...
            <Draggable draggableId={gridDraggableId(rowIndex, colIndex)} index={0}>
              {(dragProvided, dragSnapshot) => (
                <button
                  ...
                  className={`relative flex h-full min-h-[4.5rem] w-full flex-col items-center justify-center p-1 transition ejay-block ${theme.blockClass} ${
                    isPlayheadCol ? "ring-2 ring-[#7fff7f] ring-inset" : ""
                  } ${dragSnapshot.isDragging ? "ejay-cartridge-dragging opacity-90" : ""} ${
                    isPlacedFeedback ? "ejay-block-placed" : ""
                  }`}
                >
            ...
            <button
              ...
              className={`flex h-full min-h-[4.5rem] w-full flex-col items-center justify-center border border-dashed p-1 transition ${
                snapshot.isDraggingOver
                  ? "ejay-drag-over border-violet-400/60 bg-zinc-800"
                  : isAiHint
                    ? "ejay-ai-hint-cell border-emerald-400/60 bg-emerald-950/40"
                    : `ejay-cell-empty ${isPlayheadCol ? "ejay-playhead-col" : "border-zinc-700"}`
              } ${isInvalidFeedback ? "ejay-invalid-drop" : ""}`}
            >
              {isAiHint && aiHint ? (
